...
export let RouterOutlet = class RouterOutlet {
    ...
    activate(nextInstruction) {
        ...
        var localProviders = tokens.map((token) => {
        ...
        var deferPromises = tokens.map((token) => injector.get(token));
        return Promise.all(deferPromises).then((data) => {
            localProviders = tokens.map((token, idx) => {
                return provide(token, {
                    useValue: data[idx]
                });
            });
